function results = ...
    continuous_prediction_external(timeseries_train,timeseries_test,...
...
for seed_ind = 1:length(seeds)
    ...
    % standard train
    [~,~,~,~,curr_standard_coef,curr_standard_int] = ...
        ridgeCPM(mats_train(:,train_inds),phenotype_train(train_inds),...
        'thresh',pthresh,'k',1,'family',1:length(train_inds),'seed',1,'run_cpm',1);
    % caricatured train
    [~,~,~,~,curr_car_coef,curr_car_int] = ...
        ridgeCPM(mats_car_train(:,train_inds),phenotype_train(train_inds),...
        'thresh',pthresh,'k',1,'family',1:length(train_inds),'seed',1,'run_cpm',1);

    % standard test
    curr_pred = nan(length(all_subj_test),1);
    curr_pred(pred_subj_test) = ...
        mats_test'*curr_standard_coef+curr_standard_int;
    % caricatured test
    curr_pred_car = nan(length(all_subj_test),1);
    curr_pred_car(pred_subj_test) = ...
        mats_car_test'*curr_car_coef+curr_car_int;
