function results = ...
    binary_prediction_combined(timeseries,eigenvectors,family,phenotype,...
    pthresh,seeds)
%% builds predictive models for combined standard/caricatured data vs binary phenotypes
...
%% Combine the data
mats_combined = [mats; mats_car];

%% Initialize results
results = struct;
results.pred = nan(length(all_subj),length(seeds));
...
for seed_ind = 1:length(seeds)
    ...
    curr_pred(pred_subj) = cpm_classifier_family_cv(mats_combined(:,pred_subj),...
        phenotype,10,pthresh,'svm',0,seed,family,1);
    
    results.pred(:,seed_ind) = curr_pred;
end
