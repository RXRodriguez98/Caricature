function results = ...
    continuous_prediction_nulls(timeseries,eigenvectors,family,phenotype,...
    pthresh,seeds)
%% builds null predictive models for standard and caricatured data vs continuous phenotypes
...
for seed_ind = 1:length(seeds)
    ...
    [~,~,~,curr_pred(pred_subj)] = ridgeCPM(mats(:,pred_subj),null_pheno,...
        'thresh',pthresh,'k',10,'family',family,'seed',seed);
    % caricatured
    curr_pred_car = nan(length(phenotype),1);
    [~,~,~,curr_pred_car(pred_subj)] = ridgeCPM(mats_car(:,pred_subj),null_pheno_car,...
        'thresh',pthresh,'k',10,'family',family,'seed',seed);
